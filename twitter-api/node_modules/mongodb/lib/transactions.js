...
class Transaction {
    ...
    constructor(options) {
        ...
        if (writeConcern) {
            if (writeConcern.w === 0) {
                throw new error_1.MongoDriverError('Transactions do not support unacknowledged write concern');
            }
    ...
    transition(nextState) {
        ...
        throw new error_1.MongoDriverError(`Attempted illegal state transition from [${this.state}] to [${nextState}]`);
    }
    /** @internal */
    pinServer(server) {
        if (this.isActive) {
